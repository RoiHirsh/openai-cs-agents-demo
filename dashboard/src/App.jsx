...
function ResultsVideosTab() {
  ...
  async function handleDelete(r) {
    if (!confirm(`Delete the video for "${r.market}"? This cannot be undone.`)) return
    try {
      await apiFetch(`/knowledge/results-videos/${encodeURIComponent(r.market)}`, { method: 'DELETE' })
      await load()
    } catch (err) {
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.market}>
                ...
                <td className="cell-actions">
                  <button className="btn-icon" title="Delete" onClick={() => handleDelete(r)}>🗑️</button>
                </td>
              </tr>
            ))}
          </tbody>
